...
def validate_master_chef_shared_contract(repo_root: Path) -> None:
    ...
    require_substrings(
        ...
        (
            ...
            "If the runtime cannot expose live Builder reasoning or streaming partial output",
    ...
    require_substrings(
        ...
        (
            ...
            "about 10 minutes is the default quiet-work window when the approved Builder effort is clearly high-latency",
            "Apply the chosen quiet-work window only after `builder_phase` reaches `running`.",
            "If Builder replies with a coherent discovery note, partial status, or other non-final report, treat that as proof of life",
        ),
        runbook_md,
...
def validate_codex_adapter(repo_root: Path) -> None:
    ...
    require_substrings(
        ...
        (
            ...
            "should not claim live access to Builder thinking or guaranteed streaming partial output",
...
def validate_openclaw_adapter(repo_root: Path) -> None:
    ...
    require_substrings(
        ...
        (
            ...
            "Record `builder_phase: booting` as soon as the spawn request succeeds.",
            ...
            "If the runtime does not expose live Builder thinking or streaming partial output, report Builder state as `running` or `unknown` during quiet periods rather than guessing.",
