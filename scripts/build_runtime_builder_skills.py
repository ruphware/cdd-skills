"""Build runtime-specific internal Builder skills from the canonical cdd-* pack.

Examples:
  python3 scripts/build_runtime_builder_skills.py --runtime openclaw --output /tmp/runtime-builder-skills
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse CLI args for runtime-specific Builder skill generation."""
    parser = argparse.ArgumentParser(
        description=(
            "Generate runtime-specific internal Builder skill variants from the "
    ...
    parser.add_argument(
        ...
        required=True,
        choices=("openclaw",),
        help="Runtime flavor to generate.",
    )
    parser.add_argument(
        "--output",
        required=True,
        help="Directory where generated Builder skills will be written.",
    )
...
def runtime_display_name(runtime: str) -> str:
    """Return the human-readable runtime label used in rewritten docs."""
    if runtime == "openclaw":
        return "OpenClaw"
    raise ValueError(f"unsupported runtime: {runtime}")


def rewrite_description(line: str, runtime: str) -> str:
    ...
    return (
        'description: "'
        + f"{description} (internal {runtime_display_name(runtime)} Builder skill)."
        + '"'
    )


def rewrite_frontmatter(lines: list[str], runtime: str) -> list[str]:
    """Produce runtime-compatible internal-use frontmatter for one Builder skill."""
    rewritten: list[str] = []
    saw_description = False

    for line in lines:
        if line.startswith("description:"):
            rewritten.append(rewrite_description(line, runtime))
...
def rewrite_body(skill_name: str, body: str, runtime: str) -> str:
    """Add the runtime internal-use wrapper and normalize phrasing."""
    ...
    preamble = "\n".join(
        [
            "",
            f"> Internal {runtime_name} Builder variant generated from the canonical `skills/` pack.",
            ">",
...
def write_skill(output_root: Path, skill_dir: Path, runtime: str) -> None:
    ...
    rewritten = "\n".join(
        [
            "---",
            *rewrite_frontmatter(frontmatter_lines, runtime),
            "---",
            rewrite_body(skill_name, body, runtime).rstrip(),
            "",
        ]
    )
...
def build(output_root: Path, runtime: str) -> None:
    """Generate all runtime Builder skills into the requested output directory."""
    root = repo_root()
    ...
    for skill_dir in canonical_skill_dirs(skills_root):
        write_skill(output_root, skill_dir, runtime)


def main(argv: list[str] | None = None) -> int:
    """Generate the runtime Builder skill pack and print the output directory."""
    args = parse_args(argv)
    output_root = Path(args.output).resolve()
    build(output_root, args.runtime)
    print(f"Generated {runtime_display_name(args.runtime)} Builder skills -> {output_root}")
    return 0
