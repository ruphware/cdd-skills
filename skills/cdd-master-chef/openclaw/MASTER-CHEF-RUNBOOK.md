...
Builder lifecycle semantics remain shared. Use this runbook only for the OpenClaw surfaces that implement that lifecycle.
...
3. If this is a fresh run from a long-lived branch and kickoff approved a descriptive worktree branch, keep the source checkout on its original branch and use that approved branch name as `worktree_branch`.
...
6. If no descriptive worktree branch was approved, use a fresh per-run branch from the current branch `HEAD`, preferably:

   ```text
   master-chef/<run-id>
   ```

7. Create the managed worktree from the source checkout with `worktree_branch` using `git worktree add <path> -b <branch> HEAD`. Do not switch the source checkout onto `worktree_branch` as part of normal kickoff.
...
```json
{
  ...
  "builder_stop_reason": "session_closed|process_exit|builder_blocked|builder_failure|unusable_drift",
  "builder_stop_classification": "missing_requirements|solvable_blocker|route_drift|unrecoverable|pending",
  "builder_stop_evidence_summary": "<summary-or-empty>",
