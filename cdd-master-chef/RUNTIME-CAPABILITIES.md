...
| --- | --- | --- | --- | --- | --- | --- | --- |
| OpenClaw | OpenClaw subagent Builder under one Master Chef control loop | Internal `cdd-*` skills installed under `~/.openclaw/skills` | Adapter-defined; keep one Master Chef loop and single-step Builder runs | Adapter-defined in the OpenClaw runtime | Adapter-defined by the runtime/session | Provision managed worktree, then stop with relaunch instructions before implementation begins | Current packaged runtime adapter in this repo |
