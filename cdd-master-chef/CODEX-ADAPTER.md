...
- `Quiet periods:` a quiet agent, missing diff, or empty `builder.jsonl` is not enough by itself to prove that Builder has died. A timed-out wait or one unanswered progress request is still inconclusive unless Codex also reports closure or failure. Master Chef keeps active checks at least every 5 minutes while it is waiting. If Builder is still booting 10 minutes after `builder_spawn_requested_at_utc` with no readiness signal, send one final explicit boot-status probe and then treat the attempt as timed out. If Builder is running and 20 minutes pass without direct proof of life, classify it as suspect and send an explicit status probe rather than replacing it immediately. Replace only after 30 minutes of total running silence, 2 consecutive unanswered explicit probes after suspect classification, or earlier direct failure or closure evidence. Any coherent Builder reply, including discovery-only status, is proof of life rather than proof of death. When progress is uncertain, prefer direct runtime status, final agent messages, or one explicit progress request over guesswork.
