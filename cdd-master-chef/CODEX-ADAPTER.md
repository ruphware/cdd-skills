...
- `Codex direct-status surfaces:` Master Chef's direct visibility into a Codex Builder is limited to runtime-reported completion or failure notifications, explicit progress replies from the child agent, and runtime-reported closure or error events. A `wait` result that reports no agent has completed yet means `running` or `unknown`, not `dead`. Live chain-of-thought and streaming partial output are not exposed. When progress is uncertain, prefer those direct surfaces or one explicit progress request over guesswork. A quiet agent, missing diff, or empty `builder.jsonl` is not enough by itself to prove that Builder has died. Any coherent Builder reply, including discovery-only status, is proof of life rather than proof of death.
...
- `Codex session close:` when the active or a superseded Builder agent is no longer needed, close or purge that child agent through the runtime's session-management surface; preserve lineage and durable evidence first so only one live Builder remains visible.
