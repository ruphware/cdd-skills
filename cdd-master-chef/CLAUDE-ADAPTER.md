...
- `Claude direct-status surfaces:` Master Chef's direct visibility into a Claude Builder is limited to runtime-reported completion or failure notifications, explicit progress replies from the subagent, and runtime-reported closure or error events. Live chain-of-thought and streaming partial output are not exposed. When progress is uncertain, prefer those direct surfaces or one explicit progress request over guesswork. A quiet agent, missing diff, or empty `builder.jsonl` is not enough by itself to prove that Builder has died. Any coherent Builder reply, including discovery-only status, is proof of life rather than proof of death.
- `Claude probe form:` send the explicit status probe as a foreground turn in the same Claude session that owns the Builder subagent; use the agent-surface controls (`--agent` / `@`-mention / configured agent) to address the active Builder rather than spawning a new subagent.
- `Claude session close:` when the active or a superseded Builder subagent is no longer needed, end the subagent's foreground turn or let the parent session close it; preserve lineage and durable evidence first so only one live Builder remains visible.
