...
description: |
  EXPLICIT-ONLY SKILL.
  Use this skill only when explicitly invoked via `$cdd-write-prd`.
...
# CDD: Write/update PRD (explicit-only)
...
- If the PRD file is missing, create it from the template in `assets/prd.template.md`.
...
Provide:
- A PRD draft (full text) OR a minimal diff-style patch.
- A short “what changed / what’s still open” summary.

## Apply pass (after approval)
- Write the approved PRD to `docs/specs/prd.md`.
- Re-run placeholder checks if relevant (e.g., no `<PROJECT NAME>` remnants).
