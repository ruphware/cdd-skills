---
name: cdd-plan-refactor
description: |
  EXPLICIT-ONLY SKILL.
  Use this skill only when explicitly invoked via `$cdd-plan-refactor`.

  Purpose:
  - Read `docs/INDEX.md` and extract refactoring targets.
  - Propose 2–3 approaches per target with tradeoffs; recommend one.
  - Write a dedicated refactor TODO file after approval.

  Default file scope:
  - `TODO-refactoring-<tag>.md` (tag is user-provided)
  - `TODO.md` (link new TODO from an Active Work Index)
---

# CDD: Plan a refactor (INDEX-driven, explicit-only)
...
5) Read the **top** of `/docs/JOURNAL.md` for process rules.
...
- For each target:
  - Propose 2–3 architectural approaches (e.g., pipeline, layers/lasagna, micro-kernel) with brief tradeoffs.
...
- Add a link to it from `TODO.md` (Active Work Index) if present; otherwise propose adding that section and ask approval.
